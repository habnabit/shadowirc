...
/*	------------------------------------------------------------------------------------------
		...
		е If you don't use these API functions, you will also disrupt ShadowIRC's floating windows mechanism.
		...
		е And the exception to the previous rule...
...
/*	Creates a new dialog for a plugin.
		...
		Note:	е If the user hits return, you get a pUIDialogItemHit for item #1. If the user hits
...
/*	Creates a new window for a plugin.
		...
		Note:	е The window is not displayed until you explicitly do so. It is positioned behind all other windows.
*/
...
#pragma mark е Offscreen Utilities
/*	------------------------------------------------------------------------------------------
		Offscreen Utilities
...
/*	Creates a new offscreen drawing area.
		...
		Notes:	е Once you issue a StartDrawingOffscreen() call, all drawing will be made to the GWorld until you change
						the graphics port or you call End/AbortDrawingOffscreen()
					е	If there is not enough RAM to generate a GWorld of the requested size, you will draw to the screen, rather
						than to the GWorld.
*/
...
/*	Writes data to a data fork starting at the current file position.
		...
		Notes:	е Don't give PFWrite() a resource fork.
					е Equivilant to err = FSWrite(refNum, &size, p);
*/
...
/*	Reads data from a data fork starting at the current file position.
		...
		Output:	size - amound of data actually read in
					return value: File Manager error code
		Notes:	е Don't give PFRead() a resource fork.
					е Equivilant to err = FSRead(refNum, &size, p);
*/
...
/*	Gets the size of a file
		...
		Output:	size - size of file
					return value: File Manager error code
		Note:	е Equivilant to err = GetEOF(refNum, &size);
*/
...
/*	Sets the size of a file
		...
		Note:	е Equivilant to err = SetEOF(refNum, newSize);
*/
...
/*	Sets the file positon (where data is read/written from)
		...
		Output:	return value: File Manager error code
		Note:	е Equivilant to err = SetFPos(refNum, fsFromStart, pos);
*/
...
/*	Does ignore list matinence. Equivilant to /ignore command.
		Input: 	in - parameters for /ignore. If the first character is 'е', ShadowIRC will not output the ignore list.
		Output:	none
*/
